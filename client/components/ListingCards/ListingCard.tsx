...
const ListingCard = (props: any) => {
  ...
  const onViewBtnClickHandler = (incomingId: string) => {
    props.userSelectedListing(incomingId);
  };
        ...
        <div className="flex flex-col text-center lg:justify-around col-span-12 md:col-span-2 lg:col-span-2 border my-2 mx-4 py-5 md:border lg:border-0 lg:border-l lg:border-l-slate-300 dark:border-l-slate-600 border-dotted">
          ...
          <div className="">
            {soldOut ? (
              ...
              <>
                <button
                  onClick={() => onViewBtnClickHandler(_id)}
                  className={
                    `w-full flex justify-center items-center text-xl bg-brandColor  text-white hover:bg-white  hover:text-[#36106a] hover:ease-in-out border hover:border-brandColor p-2 px-6 mx-2 rounded-lg  dark:hover:bg-slate-900 dark:hover:text-white dark:hover:border-white dark:bg-slate-900 ` +
                    `${!props.userLoggedIn ? "cursor-not-allowed opacity-40" : ""}`
                  }
                >
                  View
                ...
                {!props.userLoggedIn && (
                  <span className="m-0 block p-0 pt-5 text-red-800 animate-pulse">Login To View the product.</span>
                )}
              </>
            )}
          </div>
